...
const Sell = ({ SetScreenNumber , chartData , SellLimit , HandleToast }) => {
          ...
          var UpdatedWallet = Number(localStorage.getItem("Wallet")) + (Value * Quentity);
            ...
            if(response.status == 200){
              ...
              await fetch(`${BackendBASE_URL}/PurchaseShare`, {
                ...
                body: JSON.stringify({ UserId : UserId , ShareId : ShareId , PurchsePrice : Value , Quantity : Quentity , PurchaseType : "Paid Sell" }),
              });

              var ShareName = queryParams.get("Name");

              HandleToast(`You Have Successfully Sell ${Value}Nx Price ${Quentity} ${ShareName} Quentity`);
...
<h3 style={{ fontSize: "16px" , fontWeight: "bold" , marginBottom: "8px" , color: "#f7fafc" }} >
    ...
    alignItems: "center",}}>Sell Quantity Cannot Be Less Than 1</span>
         </div>
    ...
    border: "none",
    cursor: "pointer",
    width: "100%",}} onClick={handleVerifyPay} disabled={Quentity == ""}>Sell</button>
